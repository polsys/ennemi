...
def _estimate_single_mi(x: np.ndarray, y: np.ndarray, k: int = 3) -> float:
    ...
    # Create the 2D tree for finding the k-th neighbor
...
def _estimate_conditional_mi(x: np.ndarray, y: np.ndarray, cond: np.ndarray, 
    ...
    # NOTE: cKDTree uses non-strict inequality whereas the algorithm uses strict.
    # Therefore we decrease the radius by a tiny bit.
    # TODO: Properly adapt the algorithm to this case, it would be slightly better anyways.
